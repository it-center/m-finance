...
class formController{
    ...
    public function producerSendData($f3)
	{
		...
		if (mb_strlen($arrayData[login]) < 1)             { $textError = "Вы не заполнили поле Логин"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[login]) > 255)       { $textError = "Вы ввели больше 255 символов в поле Логин"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[email]) < 1)         { $textError = "Вы не заполнили поле Электронная почта"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[email]) > 255)       { $textError = "Вы ввели больше 255 символов в поле Электронная почта"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[password]) < 1)      { $textError = "Вы не заполнили поле Пароль"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[password]) > 255)    { $textError = "Вы ввели больше 255 символов в поле Пароль"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[name]) < 1)          { $textError = "Вы не заполнили поле ФИО"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[name]) > 255)        { $textError = "Вы ввели больше 255 символов в поле ФИО"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[phone]) < 1)         { $textError = "Вы не заполнили поле Телефон"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[phone]) > 255)       { $textError = "Вы ввели больше 255 символов в поле Телефон"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (($arrayData[country]!='ru')&&($arrayData[country]!='es')&&
                ($arrayData[country]!='gb')&&($arrayData[country]!='us')) { $textError = "Вы не заполнили поле Страна"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[company]) < 1)       { $textError = "Вы не заполнили поле Название компании"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[company]) > 255)     { $textError = "Вы ввели больше 255 символов в поле Название компании"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[taxid]) < 1)         { $textError = "Вы не заполнили поле ИНН"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[taxid]) > 255)       { $textError = "Вы ввели больше 255 символов в поле ИНН"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[site]) > 255)        { $textError = "Вы ввели больше 255 символов в поле Сайт"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[companyinfo]) > 255) { $textError = "Вы ввели больше 255 символов в поле Информация о компании"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif (mb_strlen($arrayData[bankdetail]) > 255)  { $textError = "Вы ввели больше 255 символов в поле Банковские данные"; \models\formAction::producerErrorSend($f3, $textError); }
        elseif ($arrayData[check]!=1)                     { $textError = "Вы не приняли Условия использования и заявление о конфиденциальности"; \models\formAction::producerErrorSend($f3, $textError); }
        else                                              { \models\formAction::producerDoneSend($f3, $arrayData); }
	}
}